...
import { LogOut, ChevronDown, Bell, User as UserIcon, Settings as SettingsIcon, Search, Menu } from "lucide-react";
    ...
    <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-border/30 bg-background/80 px-8 backdrop-blur-md lg:px-10">
      ...
      <div className="flex items-center gap-2">
        ...
        >
          ...
          <span>Search</span>
          <kbd className="ml-3 rounded-lg bg-background px-2 py-0.5 font-mono text-xs text-muted-foreground">⌘K</kbd>
        </button>
...
function UserMenu() {
  ...
  const handleLogout = async () => { setOpen(false); await logout(); router.push("/admin/login"); };
