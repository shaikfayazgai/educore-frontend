...
import { formatDate, formatPercentage } from "@/admin/lib/utils/format";
import type { ColumnDef } from "@tanstack/react-table";
import type { Assignment } from "@/admin/lib/api/types/student.types";
...
function AssignmentDetailPanel({
  ...
  const handleSubmit = () => {
    submitMutation.mutate({ courseId, assignmentId });
  };
      ...
      {!detail.submission && detail.status !== "graded" && detail.status !== "missed" && (
        <div className="rounded-xl border border-border bg-card p-6">
          ...
          <button
            ...
            disabled={uploadFiles.length === 0 || submitMutation.isPending}
            className={cn(
              "mt-4 inline-flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-medium transition-colors",
              uploadFiles.length > 0 && !submitMutation.isPending
