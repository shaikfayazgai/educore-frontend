...
import { FormField, FormTextarea } from "@/student/components/shared/forms/form-field";
import { FormSection } from "@/student/components/shared/forms/form-section";
...
import type { WorkExperience } from "@/student/lib/api/types/student.types";
...
function ProfileTab({
  ...
  profile: import("@/student/lib/api/types/student.types").StudentProfile;
}) {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="max-w-2xl space-y-6">
      ...
      <FormSection title="Academic Info" description="These fields are managed by your institution">
        <div className="grid grid-cols-2 gap-4">
          <FormField label="Student ID / Roll No." value={profile.studentId || "—"} disabled />
          ...
          {profile.programmeCode && (
            <FormField label="Programme Code" value={profile.programmeCode} disabled />
          )}
          <FormField label="Semester" value={profile.currentSemester ? `Semester ${profile.currentSemester}` : "—"} disabled />
          {profile.section && (
            <FormField label="Section" value={profile.section} disabled />
          )}
          <FormField label="Enrollment Status" value={profile.enrollmentStatus || "active"} disabled />
          {profile.gender && (
            <FormField label="Gender" value={profile.gender} disabled />
          )}
          {profile.dateOfBirth && (
            <FormField label="Date of Birth" value={profile.dateOfBirth.slice(0, 10)} disabled />
          )}
        </div>
        <p className="text-xs text-muted-foreground">
          Academic information is managed by the Registrar&apos;s Office. If any details are incorrect, please contact your admin.
        </p>
      </FormSection>
      ...
      <FormSection title="Guardian & Address" description="Emergency contact and address information">
        <div className="grid grid-cols-2 gap-4">
          <FormField
            label="Guardian Name"
...
interface NotifToggleItem {
  key: keyof import("@/student/lib/api/types/student.types").NotificationPreferences;
  label: string;
  description: string;
}
...
function NotificationsTab({
  ...
  preferences: import("@/student/lib/api/types/student.types").NotificationPreferences;
}) {
