...
import { useSkillEvolution, useSkills } from "@/student/lib/hooks/use-student";
...
export default function StudentSkillEvolutionPage() {
  ...
  const [selectedSkills, setSelectedSkills] = useState<Set<string>>(
    new Set(allSkillNames.slice(0, 5))
  );

  // Keep selected skills in sync when data loads
  useMemo(() => {
    if (allSkillNames.length > 0 && selectedSkills.size === 0) {
      setSelectedSkills(new Set(allSkillNames.slice(0, 5)));
    }
  }, [allSkillNames]);
