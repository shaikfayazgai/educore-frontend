...
import { formatRelative } from "@/faculty/lib/utils/format";
import {
  useFacultyNotifications,
  useMarkFacultyNotificationRead,
  useMarkAllFacultyNotificationsRead,
} from "@/faculty/lib/hooks/use-faculty";
import type { FacultyNotification } from "@/faculty/lib/api/types/faculty.types";
...
function NotificationDropdown() {
  ...
  const notificationsQuery = useFacultyNotifications({ pageSize: 10 });
  const markRead = useMarkFacultyNotificationRead();
  const markAllRead = useMarkAllFacultyNotificationsRead();
      ...
      >
        ...
        {unreadCount > 0 && (
          <span
            ...
            style={{ background: "linear-gradient(135deg, #E05A35 0%, #C04628 100%)" }}
          >
        ...
        >
          ...
          {notifications.length > 0 && (
            <div className="border-t border-border bg-muted/20 px-4 py-2">
              <button
                onClick={() => { setIsOpen(false); router.push("/faculty/dashboard"); }}
