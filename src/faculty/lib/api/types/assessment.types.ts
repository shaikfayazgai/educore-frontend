...
export type QuestionType =
  | "multiple_choice"   // radio — exactly one correct answer
  | "multiple_select"   // checkbox — one or more correct answers, all required
  | "true_false"
  | "short_answer";
...
export interface AssessmentQuestion extends Identifiable {
  prompt: string;
  type: QuestionType;
  /** Options array — required for multiple_choice / multiple_select, ignored for true_false/short_answer */
  options?: string[];
  /**
   * Correct answer text.
   *   • multiple_choice → the single correct option string
   *   • multiple_select → JSON-encoded array of correct option strings, e.g. `["A","C"]`
   *   • true_false → "True" | "False"
   *   • short_answer → canonical answer (matched case/whitespace-insensitive)
   */
