...
import { api } from "@/placement/lib/api/client";
import type {
  PlacementDashboard,
  PlacementStudent,
  Employer,
...
import type { PaginationMeta } from "@/placement/lib/api/types/common.types";
...
// Distinct from /api/placement/settings (which is tenant-wide). These hit
// /api/placement/me/* and only affect the current user's row.

export function usePlacementProfile() {
  return useQuery({
    queryKey: ["placement", "profile"],
    queryFn: () => api.get<PlacementProfile>("/api/placement/me/profile"),
    select: (res) => res.data,
  });
}
...
export function useUpdatePlacementProfile() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (data: Partial<PlacementProfile>) =>
      api.patch<PlacementProfile>("/api/placement/me/profile", data),
    onSuccess: () => {
...
export function usePlacementNotificationPreferences() {
  return useQuery({
    queryKey: ["placement", "notification-preferences"],
    queryFn: () =>
      api.get<PlacementNotificationPreferences>(
        "/api/placement/me/notification-preferences",
      ),
    select: (res) => res.data,
  });
}

export function useUpdatePlacementNotificationPreferences() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: (data: Partial<PlacementNotificationPreferences>) =>
      api.patch<PlacementNotificationPreferences>(
        "/api/placement/me/notification-preferences",
        data,
      ),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["placement", "notification-preferences"] });
    },
  });
}
