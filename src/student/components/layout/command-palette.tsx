...
import { Search, X, ArrowRight, BookOpen, LayoutDashboard, Radar, Briefcase, BrainCircuit, Settings, Scale } from "lucide-react";
import { useUiStore } from "@/student/lib/stores/ui-store";
import { cn } from "@/student/lib/utils/cn";
import { useCourses } from "@/student/lib/hooks/use-student";
...
const STATIC_ITEMS: PaletteItem[] = [
  { id: "dashboard", label: "Dashboard", href: "/student/dashboard", icon: <LayoutDashboard className="h-4 w-4" />, group: "Navigation" },
  { id: "academics", label: "Academics", href: "/student/academics", icon: <BookOpen className="h-4 w-4" />, group: "Navigation" },
  { id: "skills", label: "Skills & Development", href: "/student/skills", icon: <Radar className="h-4 w-4" />, group: "Navigation" },
  { id: "tutor", label: "Guided Learning", href: "/student/tutor", icon: <BrainCircuit className="h-4 w-4" />, group: "Navigation" },
  { id: "placement", label: "Placement & Jobs", href: "/student/placement", icon: <Briefcase className="h-4 w-4" />, group: "Navigation" },
  { id: "appeals", label: "Appeals", href: "/student/appeals", icon: <Scale className="h-4 w-4" />, group: "Navigation" },
  { id: "settings", label: "Settings", href: "/student/settings", icon: <Settings className="h-4 w-4" />, group: "Navigation" },
];
...
export function CommandPalette() {
  ...
  const courses = (coursesQuery.data?.courses || []) as Array<{ id: string; code: string; name: string; semester: string }>;
  ...
  const courseItems: PaletteItem[] = courses
    ...
    .map((c) => ({
      id: `course-${c.id}`,
      label: c.name,
      sublabel: `${c.code}${c.semester ? ` • ${c.semester}` : ""}`,
      href: `/student/academics/${c.id}`,
  ...
  const allItems = [...staticItems, ...courseItems];
