...
export function QuestionCard({
  ...
  const parsedCorrect: string[] = (() => {
    if (!isMS || !result) return [];
    try {
      const v = JSON.parse(result.correctAnswer || "[]");
      return Array.isArray(v) ? v.filter((x) => typeof x === "string") : [];
    } catch { return []; }
  })();
  const toggleMultiSelect = (option: string) => {
    if (showResult) return;
    const next = parsedSelected.includes(option)
      ? parsedSelected.filter((o) => o !== option)
      : [...parsedSelected, option].sort();
      ...
      {isMS && (
        <div className="ml-8 space-y-2">
          <p className="text-xs text-muted-foreground">
            Select <span className="font-medium">all</span> answers that apply.
          </p>
          {options.map((option) => {
            const isSelected = parsedSelected.includes(option);
            const isCorrectOption = showResult && parsedCorrect.includes(option);
