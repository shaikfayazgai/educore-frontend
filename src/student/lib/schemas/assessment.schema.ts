  ...
  .superRefine((q, ctx) => {
    if (q.type === "multiple_choice" || q.type === "multiple_select") {
      ...
      if (q.type === "multiple_choice") {
        ...
        // multiple_select — correctAnswer is JSON-encoded array of option strings
        let parsed: unknown;
        try { parsed = JSON.parse(q.correctAnswer); } catch { parsed = null; }
        if (!Array.isArray(parsed) || parsed.length < 1) {
          ...
          const bad = parsed.find((v) => typeof v !== "string" || !q.options!.includes(v));
          if (bad !== undefined) {
            ctx.addIssue({
              code: "custom",
              path: ["correctAnswer"],
              message: "All correct answers must match an option",
            });
          }
        }
      }
